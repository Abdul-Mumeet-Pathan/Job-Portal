    ...
    return (
        <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-lg">
                ...
                <div className="flex items-center justify-between h-16 md:h-20">
                    ...
                    <div className="flex items-center space-x-4">
                        {user && user.role === 'student' && (
                            /* Only show mail notifications for students */
                            <Popover open={mailPopoverOpen} onOpenChange={setMailPopoverOpen}>
                                ...
                                <PopoverContent className="w-80 p-0 mt-2 shadow-xl rounded-xl border border-gray-100">
                                            ...
                                            notifications.map((notification) => (
                                                ...
                                                >
                                                    <div className="flex justify-between items-start">
                                                        <h4 className="font-medium text-gray-900">{notification.company}</h4>
                        ...
                        {!user ? (
                            <>
                                <Link to="/login">
                                    <Button variant="ghost" className="text-gray-600 hover:text-blue-600 hover:bg-blue-50 px-4">
                                        Sign In
                                    </Button>
                                </Link>
                                <Link to="/signup">
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <button className="flex items-center space-x-2 focus:outline-none group">
                                                <Avatar className="h-9 w-9 border-2 border-blue-100 group-hover:border-blue-200 transition-all shadow-md">
                                        ...
                                        <PopoverContent className="w-64 p-0 mt-2 shadow-xl rounded-xl border border-gray-100">
                                            <div className="p-4 border-b border-gray-100">
                                            ...
                                            <div className="p-2">
                                                {user && user.role === 'student' && (
                                                    <Link to="/profile">
                                                        <Button variant="ghost" className="w-full justify-start space-x-2 text-gray-700 hover:bg-gray-50">
            ...
            <div className={cn(
                ...
                mobileMenuOpen ? "max-h-screen py-4 border-t" : "max-h-0"
            )}>
                <div className="container mx-auto px-4">
                    ...
                    {user ? (
                        <div className="mt-4 pt-4 border-t border-gray-100">
                            {/* Only show mobile notifications for students */}
                            {user.role === 'student' && (
                                <div className="mb-4">
                                    <Link 
                                        to="#" 
                            ...
                            {user.role === 'student' && (
                                <Link to="/profile">
                                    <Button 
                                        variant="ghost" 
                                        className="w-full justify-start space-x-2 text-gray-700 hover:bg-gray-50 mb-2"
                                        onClick={() => setMobileMenuOpen(false)}
                                    >
                                        <User2 className="h-4 w-4 text-blue-500" />
                                        <span>View Profile</span>
...
const NavLink = ({ to, icon, label }) => (
    <Link 
        to={to}
        className={cn(
            "flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-medium text-gray-600",
...
const MobileNavLink = ({ to, icon, label, onClick }) => (
    <Link 
        ...
        onClick={onClick}
        className={cn(
            "flex items-center space-x-3 px-3 py-2 rounded-lg text-base font-medium text-gray-700",
            "hover:text-blue-600 hover:bg-blue-50 transition-colors"
        )}
    >
