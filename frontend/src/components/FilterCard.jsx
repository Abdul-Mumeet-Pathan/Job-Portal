...
const salaryRanges = [
    ...
    { label: "40k-1L BDT", min: 40000, max: 100000 },
    { label: "1L-5L BDT", min: 100000, max: 500000 }
];
...
const FilterCard = () => {
    ...
    return (
        <div className='w-full bg-white p-6 rounded-lg border border-gray-200 shadow-sm'>
            <div className='mb-6'>
                <h1 className='text-xl font-semibold text-gray-900'>Filter Options</h1>
                ...
                <div className='space-y-2 pl-1'>
                    {locationOptions.map((location) => (
                        <div key={location} className='flex items-start'>
                            ...
                            />
                            <Label htmlFor={`location-${location}`} className='ml-3 text-sm text-gray-700 hover:text-gray-900 cursor-pointer'>
                                {location}
                            </Label>
                        </div>
                    ))}
                    ...
                    {industryOptions.map((industry) => (
                        <div key={industry} className='flex items-start'>
                            <Checkbox
                                ...
                                className='h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500'
                            />
                            <Label htmlFor={`industry-${industry}`} className='ml-3 text-sm text-gray-700 hover:text-gray-900 cursor-pointer'>
                                {industry}
                            </Label>
                        </div>
                    ))}
                    ...
                    {salaryRanges.map((range) => (
                        <div key={range.label} className='flex items-start'>
                            <Checkbox
                                ...
                                className='h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500'
                            />
                            <Label htmlFor={`salary-${range.label}`} className='ml-3 text-sm text-gray-700 hover:text-gray-900 cursor-pointer'>
                                {range.label}
                            </Label>
                        </div>
                    ))}
